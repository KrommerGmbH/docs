...
import template from './cmh-date-filter.html?raw';
import './cmh-date-filter.scss';
...
export default defineComponent({
    ...
    data() {
        return {
            ...
            timeframeOptions: [
                {
                    label: this.$tc('cmh-order.filters.orderDateFilter.options.lastYear'),
                    value: -365,
                },
                {
                    label: this.$tc('cmh-order.filters.orderDateFilter.options.lastQuarter'),
                    value: 'lastQuarter',
                },
                {
                    label: this.$tc('cmh-order.filters.orderDateFilter.options.lastMonth'),
                    value: -30,
                },
                {
                    label: this.$tc('cmh-order.filters.orderDateFilter.options.lastWeek'),
                    value: -7,
                },
                {
                    label: this.$tc('cmh-order.filters.orderDateFilter.options.lastDay'),
                    value: -1,
                },
                {
                    label: this.$tc('cmh-order.filters.orderDateFilter.options.custom'),
                    value: 'custom',
                    hidden: true,
                },
            ],
    ...
    methods: {
        ...
        onTimeframeSelect(timeframe) {
            ...
            if (!this.timeframeOptions.some((t) => t.value === timeframe)) {
                console.error(`Timeframe ${timeframe} is not allowed for cmh-date-filter component`);
                return;
            }
