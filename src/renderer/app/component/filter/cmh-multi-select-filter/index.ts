...
import { Criteria, EntityCollection } from '@core/data';
import template from './cmh-multi-select-filter.html?raw';
...
export default defineComponent({
    ...
    computed: {
        ...
        values() {
            ...
            const entities = new EntityCollection<Record<string, unknown> & { id: string }>(this.filter.schema.entity);

            if (Array.isArray(this.filter.value)) {
                this.filter.value.forEach((value) => {
                    const entityValue: Record<string, unknown> & { id: string; variation?: unknown } = {
                        id: value.id,
